...
static int sumOperation = 0, numOfChange=0;
...
{
    //Конструктор класса, который берет координаты левого верхнего угла квадрата из класса координат coordinates, а значению длины присваивает 0.
...
void backtracking( std::vector< std::vector<int> > &initialSquare , unsigned long sideLengthLimit, int colour, std::vector<quadrate>* answer, std::vector<quadrate> tempAnswer )
{
    ...
    bool perimeterGaps = true;   //true - нет пробелов по периметру вокруг квадртата, false - есть
    ...
    {
        backtracking(beforelast, sideLengthLimit-1, colour, answer, tempAnswer);
//        std::cout << "Разбивание №" << numOfChange << " " << std::endl;
//        printAnswer(tempAnswer);   //Вывод ответа
	sumOperation++;
	numOfChange++;
    }
...
{
    if (argc != 2){
        std::cout << "Fail with number of arguments" << std::endl;
        return 0;
    }

    int squareSideLength = atoi(argv[1]);   //Переменная, равная длине стороны главного большого квадрата
    ...
    std::cout << "Количество операций перед нахождение минимального решения:" << sumOperation << std::endl;
}
